...
export default class resizable {
	...
	resizeInitiate() {
		...
		this.sizeLimit = {
			...
			maxWidth: this.options.maxWidth !== undefined ? this.options.maxWidth : Number.POSITIVE_INFINITY,
			minHeight: this.options.minHeight !== undefined ? this.options.minHeight : 0,
			maxHeight: this.options.maxHeight !== undefined ? this.options.maxHeight : Number.POSITIVE_INFINITY,
		};
	}
	...
	resize(e) {
		...
		/**
		 * compare the new size with size limitation(given by users) to make sure it's within the size limitation.
		 */
	...
	request(resizeParameter) {
		...
		if (resizeParameter.width === undefined || resizeParameter.height === undefined) return false;
		/* start resize */
		// store the start css translate value. [x,y]
...
function cssPreProcess(style) {
	...
	result.toString = function() {
		...
		for (let selector in this) {
			if (typeof this[selector] !== "function") text += `${selector}{\n${this.stringifyItems(this[selector])}}\n`;
		}
		return text;
	};
	return result;
